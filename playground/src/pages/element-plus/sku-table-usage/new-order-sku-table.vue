...
function editable(value: string, meta: any): SkuTableCellEditableValue {
  
  const v = Number(value)
  if (v < 0) {
    return { value: '0', error: '数量不能小于0' }
  } else if (v > meta.stock) {
    return { value: `${meta.stock}`, error: `数量不能大于${meta.stock}` }
  }
  return { value }
}
